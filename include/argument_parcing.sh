...
do
    ...
    -h|--help)
     ...
     u[0-9][0-9][0-9][0-9][0-9][0-9][0-9]|user_000[0-9][0-9][0-9][0-9][0-9][0-9][0-9]|user[0-9][0-9][0-9][0-9][0-9][0-9]*)
		user_login="$1"
        #--------------------------------------------------------
		# HOME каталог пользователя
		user_dir_short=`sudo grep "${user_login}:" /etc/passwd | grep -v "^#" | awk -F: '{print $6}'` ;
		user_dir_short=`readlink ${user_dir_short}` ; # резолвим реальный каталог, если это симлинк
		user_dir_short=`echo ${user_dir_short} | sed "s#^${varwww_directory}##g"`
		if [[ -z ${user_dir_short} ]]; then
			echo -e "There is no user ${fail_mes}"
			end_script
	 ...
	 u[0-9][0-9][0-9][0-9][0-9][0-9][0-9]_*|user_0000[0-9][0-9][0-9][0-9][0-9][0-9]_*)
		let "restore_bases_count=$restore_bases_count+1"
