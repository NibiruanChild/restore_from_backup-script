...
do
	...
	if [[ "${operation_type}" == "restore" ]]; then
		...
		echo -e -n "\t\tBackuping:   "	
		# создаем дамп
        mysqldump -u${mysqladmin} -p${dbrootpass} ${restore_bases[$i]} > ~/${tmpBakDirName}/${restore_bases[$i]}_${timestamp}.sql
        ...
        mysql -u${mysqladmin} -p${dbrootpass} ${restore_bases[$i]} < ~/${tmpBakDirName}/${restore_bases[$i]}-${backup_date}.sql
        check_status $? strict
	fi
